...
// ═══════════════════════════════════════════════
// Run this script to immediately register all slash commands
// Usage: node force-register-commands.js
...
// Fallback: Hardcode values here if .env still fails (remove after testing)
// const CLIENT_ID = "1085570438853308517";  // Your bot's Application ID
// const DISCORD_TOKEN = "MTA4NTU3MDQzODg1MzMwODUxNw.GMS82b.4-HAUEoXHDv1YhvuC..."; // Full token
...
const commands = [
  ...
  // ⭐ NEWLY ADDED LEVEL COMMANDS - THESE WERE MISSING!
  { name: 'level-channel', description: 'Set a dedicated channel for level-up announcements (Admin only)', options: [{ name: 'channel', type: 7, description: 'Channel for level-up messages (leave empty to use current channel)', required: false }] },
  { name: 'level-config', description: 'Configure XP rates and leveling settings (Admin only)' },
  ...
  { name: 'toggle-levelup-mention', description: 'Toggle whether you get mentioned when you level up', options: [{ name: 'enabled', type: 5, description: 'Enable or disable level-up mentions for yourself', required: true }] },
...
async function registerCommands() {
  ...
  if (!CLIENT_ID) {
    ...
    console.log('   CLIENT_ID=1085570438853308517');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('🎉 All commands are now available globally!');
    console.log('');
    console.log('📋 Newly registered level commands:');
    console.log('   • /level-channel - Set dedicated level-up channel');
    console.log('   • /level-config - Configure XP settings');
    console.log('   • /xp-boost - Set bonus XP for actions');
    console.log('   • /no-xp-channel - Manage no-XP channels');
    console.log('   • /no-xp-role - Manage no-XP roles');
    console.log('   • /toggle-levelup-mention - Toggle level-up mentions');
    console.log('   • /personality-list - View all personality modes');
    console.log('');
    console.log('⏱️  Note: It may take a few minutes for commands to appear in Discord.');
    
  } catch (error) {
    console.error('❌ Error registering commands:', error);
    if (error.code === 50001) {
      console.error('💡 This error means the bot lacks OAuth2 permissions. Re-invite with applications.commands scope!');
    }
  }
}
